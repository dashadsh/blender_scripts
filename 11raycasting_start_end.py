...
with open(output_path, 'w') as f:
    # Take measurements
    start_dist = measure_distance(vinyl, measure_dot, 1)
    mid_dist = measure_distance(vinyl, measure_dot, 60)
    end_dist = measure_distance(vinyl, measure_dot, 120)
    
    # Write to file
    f.write("Distance Measurements:\n")
    f.write(f"Start (Frame 1): {start_dist:.3f}\n")
    f.write(f"Middle (Frame 60): {mid_dist:.3f}\n")
    f.write(f"End (Frame 120): {end_dist:.3f}\n")
